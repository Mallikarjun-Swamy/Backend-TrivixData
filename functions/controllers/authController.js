...
export const googleAuthCallback = async (req, res) => {
  try {
    ...
    res.redirect(`${FRONTEND_URL}/profile/account`);
  } catch (err) {
    console.error(err);
...
//     // Handle initial user lookup errors (user not found)
//     if (userError || !user) {
//       // Log the failed login attempt for this email, even if the user doesn't exist
//       // This helps in detecting enumeration attacks
...
//     // Check for account lock status before attempting password validation
//     if (user.account_locked && user.role !== "admin") {
//       return res.status(403).json({
//         message:
//           "Your account is locked due to multiple failed attempts. Please wait for one day or contact support.",
//       });
//     }
...
//       const shouldLockAccount = failedAttempts >= 5; // Example: lock after 5 failed attempts
...
//       const { error: err } = await supabase // ✅ The code waits here for the update to finish
//         .from("users")
//         .update(updateData)
//         .eq("id", user.id);
...
//       // ✅ The return statement is now executed AFTER the update operation is complete.
...
//     // Check email verification after successful password validation
//     if (!user.is_verified) {
//       return res.status(403).json({
//         message:
...
//     // Update last_login to current timestamp in ISO format
//     await supabase
//       .from("users")
...
//   } catch (err) {
//     // This catch block is now only for unexpected server-side errors
